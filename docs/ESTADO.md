...
spec de negocio v0.4 aprobada, todavía sin código).
...
2. **Lanzar el pedido 1a** con el orquestador (y después el 1b sobre lo entregado):
   ```js
   Workflow({
   ...
   ```
3. Cuando exista código: ajustar `.claude/launch.json` a los comandos reales y
